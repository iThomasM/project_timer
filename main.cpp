...
enum State {
  ...
  READY,
  TIMING,
  AO5_STAGE,       // New stage for Ao5
  CURRENT_TIME_STAGE  // New stage for CurrentTime
};
...
void restartBTN(int btnState, unsigned long lastBtnPress) {
  if (restartState == HIGH && (millis() - lastBtnPress) > DEBOUNCE_DELAY) {
    ...
    if (currentSelection == Ao5) {
      currentState = AO5_STAGE;
    }
    else if (currentSelection == CurrentTime) {
      currentState = CURRENT_TIME_STAGE;
    } else {
      currentState = IDLE;
    }
    
    if (currentState == AO5_STAGE || currentState == CURRENT_TIME_STAGE){
      currentState = IDLE;
    }
